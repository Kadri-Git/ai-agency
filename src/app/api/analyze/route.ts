...
import { queryChatGPT, queryClaude, queryGemini, calculateShareOfVoice, type QueryResult, type Platform } from '@/lib/ai-integrations'
...
function calculateVisibilityByPlatform(
  ...
  sovByPlatform: { chatgpt?: number; claude?: number; gemini?: number; perplexity?: number }
  ...
  for (const platform of platforms) {
    ...
    const platformMentionRate = platformMentioned.length / platformResults.length
    const platformSovValue = sovByPlatform[platform] || 0
    const platformPositions = platformMentioned
      .map((r) => r.position)
      .filter((p): p is number => p !== null && p > 0)
    const platformAvgPosition = platformPositions.length > 0 
      ? platformPositions.reduce((sum, p) => sum + p, 0) / platformPositions.length 
      : 0
    const platformFavorable = platformMentioned.filter((r) => r.sentiment === 'favorable').length
    
    visibilityByPlatform[platform] = Math.min(100,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`🔑 API Keys status: OpenAI=${hasOpenAI ? '✅' : '❌'}, Anthropic=${hasAnthropic ? '✅' : '❌'}, Google=${hasGoogle ? '✅' : '❌'}`)
    }
    
    if (!hasOpenAI && !hasAnthropic && !hasGoogle) {
      return NextResponse.json(
        { error: 'No API keys configured. Please add at least one API key (OPENAI_API_KEY, ANTHROPIC_API_KEY, or GOOGLE_AI_API_KEY) to your .env file.' },
        { status: 400 }
      )
    }
    ...
    // Generate industry-specific prompts that are more likely to mention the company
    // Make prompts very explicit to ensure the company is mentioned
    const prompts = industry 
      ? [
    ...
    for (const prompt of prompts) {
      for (const platform of platforms) {
        try {
          if (process.env.NODE_ENV === 'development') {
            console.log(`📡 Querying ${platform} with: "${prompt.substring(0, 80)}..."`)
          }
          let result
          if (platform === 'chatgpt') {
          ...
          if (result) {
            if (process.env.NODE_ENV === 'development') {
              console.log(`✅ ${platform} result: mentioned=${result.mentioned}, position=${result.position || 'N/A'}, response length=${result.response.length}`)
              if (result.mentioned) {
                console.log(`   Response snippet: ${result.response.substring(0, 150)}...`)
              } else {
                console.log(`   ⚠️ Company NOT mentioned. Response preview: ${result.response.substring(0, 200)}...`)
                console.log(`   Looking for: "${company}" or domain "${domain}"`)
              }
            }
            results.push(result)
          } else {
          ...
          console.error(`❌ Error querying ${platform} with prompt "${prompt}":`, errorMessage)
          
          // Check for specific error types
          if (errorMessage.includes('API key') || errorMessage.includes('authentication')) {
            console.error(`   ⚠️ API key issue for ${platform}. Check your .env file.`)
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`📊 Analysis complete. Total results: ${results.length}, Mentioned: ${results.filter(r => r.mentioned).length}`)
      // Log detailed results
      results.forEach((r, idx) => {
        console.log(`  Result ${idx + 1}: platform=${r.platform}, mentioned=${r.mentioned}, position=${r.position || 'N/A'}, response_length=${r.response.length}`)
        if (r.mentioned && r.response) {
          console.log(`    Response snippet: ${r.response.substring(0, 200)}...`)
        } else if (!r.mentioned) {
          console.log(`    ⚠️ NOT MENTIONED - Response preview: ${r.response.substring(0, 300)}...`)
        }
      })
      ...
      platformBreakdown.forEach(platform => {
        ...
        console.log(`  ${platform}: ${mentioned}/${platformResults.length} mentions`)
      })
    }
    ...
    const mentionRate = totalQueries > 0 ? mentionedResults.length / totalQueries : 0
    
    if (process.env.NODE_ENV === 'development') {
      console.log(`📊 Metrics: mentionRate=${(mentionRate * 100).toFixed(1)}%, mentionedResults=${mentionedResults.length}, totalQueries=${totalQueries}`)
    }
    ...
    const platformSov: { chatgpt: number; claude: number; gemini: number; perplexity: number } = {
      chatgpt: 0,
      claude: 0,
      gemini: 0,
      perplexity: 0,
    ...
    platforms.forEach(platform => {
      ...
      if (platformResults.length > 0) {
        ...
        if (process.env.NODE_ENV === 'development') {
          const mentioned = platformResults.filter(r => r.mentioned).length
          console.log(`📊 ${platform} SOV calculation: ${mentioned}/${platformResults.length} mentioned, SOV=${sov.toFixed(1)}%`)
        }
      } else {
        if (process.env.NODE_ENV === 'development') {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`📊 Platform SOV Summary: ChatGPT=${platformSov.chatgpt.toFixed(1)}%, Claude=${platformSov.claude.toFixed(1)}%, Gemini=${platformSov.gemini.toFixed(1)}%`)
      console.log(`📊 Overall SOV: ${shareOfVoice.toFixed(1)}%`)
    }
    ...
    const topCompetitorByPlatform: Record<string, { name: string; sov: number }> = {
      chatgpt: { name: '', sov: 0 },
      claude: { name: '', sov: 0 },
      gemini: { name: '', sov: 0 },
    ...
    for (const platform of platforms) {
      ...
      for (const result of platformResults) {
        if (result.competitorsMentioned && result.competitorsMentioned.length > 0) {
          for (const competitorName of result.competitorsMentioned) {
            ...
            const competitorResult: QueryResult = {
              ...
              position: result.competitorPositions?.[competitorName] || undefined,
      ...
      for (const [competitorName, competitorResults] of competitorMap) {
        ...
        const competitorSov = calculateShareOfVoice(allPlatformResultsForCompetitor)
        ...
        if (competitorSov > topCompetitorByPlatform[platform].sov) {
          topCompetitorByPlatform[platform] = { name: competitorName, sov: competitorSov }
        }
      }
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      for (const platform of platforms) {
        ...
        if (top.name) {
          console.log(`📊 Top competitor on ${platform}: ${top.name} (${top.sov.toFixed(1)}%)`)
        }
      }
    }
    ...
    const visibilityByPlatform: { chatgpt: number; claude: number; gemini: number; perplexity: number } = {
      chatgpt: 0,
      claude: 0,
      gemini: 0,
      perplexity: 0,
    ...
    const audiencePerMention = mentionedResults.reduce((sum, r) => {
      ...
      const baseAudience = pos === 1 ? 50000 : pos === 2 ? 30000 : pos === 3 ? 15000 : 5000
    ...
    const sourceDiversityScore = Math.min(10, 5 + (uniquePlatforms * 1.5) + (mentionRate * 2))
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`✅ Website analysis complete. Visibility score: ${websiteVisibilityScore}/100`)
      }
    } catch (error) {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.log(`   Mention count: ${mentionedResults.length}/${totalQueries}`)
    ...
    const analysis = await prisma.analysis.create({
      data: {
        ...
        citationCount: mentionedResults.filter((r) => r.citations && Array.isArray(r.citations) && r.citations.length > 0).length,
        favorableMentions,
        neutralMentions,
        negativeMentions,
        averagePosition,
        ...
        websiteAnalysis: websiteAnalysis ? JSON.parse(JSON.stringify(websiteAnalysis)) : null,
  ...
  try {
    ...
    if (latest && domain) {
      ...
      for (const platform of platforms) {
        ...
        for (const result of platformResults) {
          const competitorsMentioned = Array.isArray(result.competitorsMentioned) 
            ...
            : []
          const competitorPositions = result.competitorPositions && typeof result.competitorPositions === 'object'
        ...
        for (const [competitorName, competitorResults] of competitorMap) {
          ...
          const queryResultsForCompetitor: QueryResult[] = platformResults.map((result: any) => {
            ...
            return {
              ...
              response: result.response,
              mentioned: isMentioned,
              position: isMentioned ? (competitorPositions[competitorName] || undefined) : undefined,
              sentiment: 'neutral' as const,
              sentimentScore: 0,
            }
          })
      ...
      const visibilityByPlatform = calculateVisibilityByPlatform(
        analysis.results,
        analysis.sovByPlatform as { chatgpt?: number; claude?: number; gemini?: number; perplexity?: number }
      )
