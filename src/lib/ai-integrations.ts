...
export async function queryChatGPT(
  ...
  for (const modelName of openaiModels) {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `✅ ChatGPT (${modelName}) response received (${content.length} chars). Checking for mention of "${companyName}"...`
        )
      }

      const mentioned = detectMention(content, companyName, companyDomain)
      const { competitors, positions } = extractCompetitors(
        content,
        companyName,
        companyDomain
      )
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `🔍 ChatGPT mention detection: ${mentioned ? '✅ MENTIONED' : '❌ NOT MENTIONED'}`
        )
      }
      ...
      return {
        ...
        contextSnippet: mentioned
          ? extractContextSnippet(content, companyName)
          : undefined,
        sentiment: mentioned
          ? analyzeSentiment(content, companyName)
          : undefined,
        sentimentScore: mentioned
          ? calculateSentimentScore(content, companyName)
          : undefined,
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `❌ ChatGPT model ${modelName} failed: ${error instanceof Error ? error.message : String(error)}`
        )
      }
      // Try next model
      continue
  ...
  // All models failed
  console.error(
    'ChatGPT query error: All models failed. Last error:',
    lastError
  )
...
export async function queryClaude(
  ...
  // Note: Model availability depends on API key access and region
  const claudeModels = [
    'claude-3-5-haiku-20241022', // Fastest, most available
    'claude-3-5-sonnet-20241022', // Latest Sonnet version
    'claude-3-5-haiku', // Without date suffix
  ...
  for (const modelName of claudeModels) {
    try {
      if (process.env.NODE_ENV === 'development') {
        console.log(`🔍 Querying Claude with model: ${modelName}`)
      }
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `✅ Claude (${modelName}) response received (${content.length} chars). Checking for mention of "${companyName}"...`
        )
      }
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `🔍 Claude mention detection: ${mentioned ? '✅ MENTIONED' : '❌ NOT MENTIONED'}`
        )
      }
      ...
      // Don't retry on 404 errors - model doesn't exist
      if (errorMessage.includes('404') || errorMessage.includes('not_found')) {
        if (process.env.NODE_ENV === 'development') {
          console.log(
            `❌ Claude model ${modelName} not found (404) - trying next model...`
          )
        }
        continue
      }
      ...
      if (
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`❌ Claude quota/rate limit exceeded - stopping retries`)
        }
        throw error
      }

      if (process.env.NODE_ENV === 'development') {
        console.log(`❌ Claude model ${modelName} failed: ${errorMessage}`)
      }
  ...
  // All models failed - return a graceful error response instead of throwing
  // This allows the analysis to continue with other platforms
  console.warn(
    '⚠️ Claude query failed: All models unavailable. Continuing with other platforms.'
  )
  return {
    platform: 'claude',
    prompt,
    response: `Error: All Claude models failed. Last error: ${lastError?.message || 'Unknown error'}`,
    mentioned: false,
    position: undefined,
    contextSnippet: undefined,
    sentiment: undefined,
    sentimentScore: undefined,
    competitorsMentioned: undefined,
    competitorPositions: undefined,
  }
}
...
export async function queryGemini(
  ...
  if (!genAI) {
    throw new Error(
      'Google AI API key not configured. Please add GOOGLE_AI_API_KEY to your .env file.'
    )
  }

  // Use the best working Gemini model
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(
      `✅ Gemini (${modelName}) response received (${content.length} chars). Checking for mention of "${companyName}"...`
    )
  }
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(
      `🔍 Gemini mention detection: ${mentioned ? '✅ MENTIONED' : '❌ NOT MENTIONED'}`
    )
  }
...
function detectMention(
  ...
  const lowerDomain = companyDomain
    .toLowerCase()
    .replace(/^https?:\/\//, '')
    .replace(/^www\./, '')
  ...
  if (techCompanyPatterns[baseName]) {
    for (const pattern of techCompanyPatterns[baseName]) {
      if (lowerContent.includes(pattern)) {
        if (process.env.NODE_ENV === 'development') {
          console.log(
            `✅ Mention found: tech company pattern "${pattern}" in content`
          )
        }
        return true
      }
  ...
  if (commonWords.has(baseName)) {
    ...
    if (exactMatches >= 2) {
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `✅ Mention found: "${baseName}" appears ${exactMatches} times (common word, multiple mentions)`
        )
      }
      return true
    }
    ...
    const hasContext = companyContext.some((ctx) => {
      const contextPattern = new RegExp(
        `\\b${escapedBaseName}[^a-z]*${ctx}\\b|\\b${ctx}[^a-z]*${escapedBaseName}\\b`,
        'i'
      )
  ...
  const namePattern = new RegExp(
    `\\b${escapedBaseName}(?:'s|'| inc\\.?| inc| corporation| corp\\.?| corp| company| co\\.?| co| technologies| tech)?\\b`,
    'i'
  )
  if (namePattern.test(content)) {
    if (process.env.NODE_ENV === 'development') {
      console.log(
        `✅ Mention found: pattern match for "${baseName}" in content`
      )
    }
    return true
  }
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(
      `❌ No mention found. Looking for: "${companyName}" (${lowerName}) or domain "${lowerDomain}"`
    )
...
function extractCompetitors(
  ...
  const potentialCompanies = new Map<
    string,
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (listMatch) {
      ...
      const companyName = extractCompanyNameFromLine(
        itemText,
        targetCompanyName,
        targetCompanyDomain
      )
      if (
        companyName &&
        companyName.toLowerCase() !== lowerTargetName &&
  ...
  const sentencePattern =
    /([A-Z][a-zA-Z]+(?:\s+(?:Inc|Corp|Ltd|LLC|Co|Company|Technologies|Tech|Systems|Solutions|Group|Industries|International|Global|Oyj|AB|ASA))?\.?)(?:\s*[,;]\s*|(?:\s+and\s+))?/g
  let match
  ...
  for (const [_, value] of potentialCompanies) {
    ...
    if (
      geographicTerms.has(lowerName) ||
      excludeWords.has(lowerName) ||
      geographicTerms.has(firstWord) ||
      excludeWords.has(firstWord)
    ) {
      continue
    }
    ...
    if (
      ...
      lowerName.includes('major') ||
      lowerName.includes('best') ||
      lowerName.includes('largest') ||
      lowerName.includes('notable')
    ) {
      continue
    }
  ...
  if (process.env.NODE_ENV === 'development' && competitors.length > 0) {
    console.log(
      `🔍 Extracted competitors for ${targetCompanyName}:`,
      competitors
    )
...
function extractCompanyNameFromLine(
  ...
  for (let i = 0; i < Math.min(words.length, 4); i++) {
    ...
    if (
      word.toLowerCase().includes(targetLower.split(' ')[0]) ||
  ...
  if (companyWords.length > 0) {
    ...
    if (
      descriptionWords.some(
        (desc) => lowerName === desc || lowerName.startsWith(desc + ' ')
      )
    ) {
      return null
    }
 ...
 */
function findCompanyPositionInContent(
  content: string,
  companyName: string
): number | null {
  const lines = content.split('\n')
...
export function calculateShareOfVoice(
  ...
  const avgPosition =
    mentionedResults.reduce((sum, r) => sum + (r.position || 5), 0) /
  ...
  const favorableCount = mentionedResults.filter(
    (r) => r.sentiment === 'favorable'
  ).length
  const negativeCount = mentionedResults.filter(
    (r) => r.sentiment === 'negative'
  ).length
  const sentimentScore =
    (favorableCount / mentionedResults.length) * 20 -
    (negativeCount / mentionedResults.length) * 10
  ...
  // Total SOV (0-100)
  const totalSOV =
    mentionRateScore + positionScore + adjustedSentimentScore + platformScore
